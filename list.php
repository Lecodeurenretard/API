        ...
        if(isMaxWeightAndAvailable('application/json') || isMaxWeightAndAvailable('application/*') || isMaxWeightAndAvailable('*/*')){
            ...
            $ret = '[' . PHP_EOL; 
                foreach(glob(MUSIC::STORAGE_PATH . '/*.mp3') as $file){
                    $request = curl_init("http://api.musiques.nils.test.sc2mnrf0802.universe.wf/get-json.php?indent=1&file=" . urlencode(basename($file, '.mp3')));
            ...
            return $ret;
        }elseif(isMaxWeightAndAvailable('application/xml')){
            header('Content-Type: application/xml; charset=utf-8', true, 200);

            $ret = '<musics>' . PHP_EOL; 
                foreach(glob(MUSIC::STORAGE_PATH . '/*.mp3') as $file){
                    $request = curl_init("http://api.musiques.nils.test.sc2mnrf0802.universe.wf/get-json.php?indent=2&file=" . urlencode(basename($file, '.mp3')));
                    curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
